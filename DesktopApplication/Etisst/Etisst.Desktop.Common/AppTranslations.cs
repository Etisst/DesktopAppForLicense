
namespace Etisst.Desktop.Common
{
   // [System.Runtime.InteropServices.GuidAttribute("76DF2981-3387-429E-8AD2-BEBFB4C79132")]

    public class AppTranslations
    {
        public const string MUST_SELECT_PRODUCT = "You need to select the product first!";
        public const string DELETE_PRODUCT = "Are you sure you want to delete this product?";
        public const string PRODUCT = "Product";

        public const string MUST_SELECT_PRODUCT_CATEGORY = "You need to select product category first!";
        public const string DELETE_PRODUCT_CATEGORY = "Are you sure you want to delete this product category?";
        public const string MUST_COMPLETE_ALL_MANDATORY_FIELDS = "Please complete all mandatory fields!";
        public const string PRODUCT_CATEGORY = "Product category";

        public const string MUST_SELECT_PRODUCT_TAG = "You need to select product tag first!";
        public const string DELETE_PRODUCT_TAG = "Are you sure you want to delete this product tag?";
        public const string PRODUCT_TAG = "Product tag";

        public const string PRODUCT_SHORT_DESCRIPTION = "<h2>Short description:</h2>";
        public const string PRODUCT_DESCRIPTION = "<h2>Description:</h2>";
    }
}
